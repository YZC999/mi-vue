<template>
  <div class="qy_topBanner">
    <div class="topBanner_all">
      <div class="qy_lunboLeft" @mouseleave="leaveAlone">
        <ul>
          <li v-for="item in sideItems" @mouseenter="currGoodschange(item.type)">
      ...
      <div class="category-detail" v-show="isShow">
      	<ul class="category-items" v-for="goods in currGoods">
      		<li class="category-goods" v-for="item in goods">
      			...
      			<router-link to="" class="goods-buy-link">选购</router-link>
      		</li>
      	</ul>
      </div>

      <!-- 轮播 -->
      <myslide  class="qy_lunboRight"></myslide>
    </div>
  </div>
</template>

<script>
import myslide from './common/slide.vue';
...
export default {
  ...
  methods: {
      ...
      // 向上取整Math.ceil()
      console.log(curtype);
      let mytype = this[curtype]
  		const lengthUl=Math.ceil(mytype.length/6);//得出组数
  		for(var i=0;i<lengthUl;i++){//以6个数据为一组列出数据
  			var arr=[];
  			for(let j=i*6;j<6*(i+1);j++){
  				if(mytype[j]){//数据不为空才push
  				arr.push(mytype[j]);
  				}
  			}
  			this.currGoods.push(arr);
  			arr=[];
  		}
  		// console.log(this.currGoods);
      // return this.currGoods;
  	},
    leaveAlone (){
      this.isShow=false;
    }
...
<style>
    ...
    .topBanner_all .qy_lunboLeft li:hover{
      background: #ff6700;
    }
    .topBanner_all .qy_lunboRight{
      position: absolute;
      top: 0;
      right:0;
      width: 1226px;
    ...
    .category-detail .category-goods{
    	width: 265px;
    	height: 76.6px;
    	display: flex;
    ...
    .category-detail .category-goods .goods-link{
    	width: 170px;
    	display: flex;
		  align-items: center;
    ...
    .category-detail .category-goods .goods-buy-link{
    	display: block;
    	width: 58px;
    	height: 22px;
    	line-height: 22px;
    ...
    .category-detail .category-goods .goods-buy-link:hover{
    	color: #fff;
    	background: #ff6700;
    }
